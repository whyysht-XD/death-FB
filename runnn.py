...
def menu(my_name,my_id):
	...
	au='[-] AUTHOR : WAHYU CANDRA PRATAMA\n[-] FACEBOOK : Wahyu Candra Pratama\n[-] INSTAGRAM : whyysht_'                                             
...
def error():
	...
	time.sleep(4)
	os.system("xdg-open https://www.facebook.com/wahyu.go.id")
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r LOADING >_ {P}{b}{loop}{P}/{u}{len(id)}{P} OK {P}{H}{ok}{P} CP {P}{k}{cp}{x} : {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "), 
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r└──>[WHYYSHT] {P}{b}{loop}{P}/{u}{len(id)}{P} OK {P}{H}{ok}{P} CP {P}{k}{cp}{x} : {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "), 
	sys.stdout.flush()
	nip=random.choice(prox)
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/WHYYSHT-XD')
	except:pass
	try:os.system('touch .prox.txt')
	except:pass
	try:os.system('clear')
	except:pass
	ilman(f'\n\t {h}HALLO WELCOME THIS IS SCRIPT WAHYU CANDRA PRATAMA, ENJOY USE THIS SCRIPT :>{x} ')
	time.sleep(3)
	login()
